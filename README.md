...
По типу `ItemRes extends Resource` --> `MaterialItemRes extends ItemRes`, где, например `ItemRes`:
...
И вот про категории. В Godot тупо нельзя сделать вот так:

```ts
type Category = "tool" | "material" | "etc";
```

Чтобы потом в предмете сделать что-то типа:

```ts
type MaterialItem = { ..., category: Category }
const item: MaterialItem = { ..., category: "tool" }
```

И IntelliSense тебе говорит, что у тебя на выбор 3 варианта и больше никак. Ну хоть есть какието ограничения при задании `@export`
...
```gdscript
@export_enum("placeable", "something", "anything") var category: String = ""
```
